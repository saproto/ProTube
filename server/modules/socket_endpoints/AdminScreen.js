...
eventBus.on("volume-update", (newVolume) => {
  endpoint.emit("volume-update", newVolume);
});
eventBus.on("new-screen-code", (screenCode) => {
  endpoint.emit("new-screen-code", screenCode);
});
