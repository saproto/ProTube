...
onMounted(() => {
    ...
    if (!textWidth || !parentWidth) return false;
    textOverflowing.value = textWidth > parentWidth;
});

defineEmits(['video-clicked', 'remove-clicked']);
    ...
    <transition appear @before-enter="beforeEnter" @enter="enter">
        ...
        >
            <div
                ...
                class="bg-secondary absolute bottom-0 h-2 w-0 rounded-sm opacity-60"
            ></div>
            <button
                :disabled="!clickable"
                :class="[
                    clickable && statusIcon === 0 ? 'hover:bg-white/40 dark:hover:bg-stone-800/40' : 'cursor-default',
                    { 'rounded-lg': roundedCorners },
                ]"
                class="border-secondary flex flex-1 flex-col border-l-4 bg-white/80 px-8 py-4 duration-200 dark:border-gray-800 dark:bg-stone-800/80"
                @click="$emit('video-clicked')"
            >
                <div class="text-md w-full overflow-x-hidden text-lg font-bold text-gray-800 dark:text-stone-300">
                    <h3
                        :id="cardId"
                        :class="{
                ...
                >
                    ...
                    <li v-if="channel" class="flex flex-1 text-right">
                        <span class="fa-li">
                            <font-awesome-icon icon="fa-solid fa-microphone" fixed-width> </font-awesome-icon>
                        </span>
                        <span class="truncate">
                            {{ channel }}
                        </span>
                    ...
                    <li v-if="views" class="flex w-full flex-1">
                        <span class="fa-li">
                            <font-awesome-icon icon="fa-solid fa-eye" fixed-width> </font-awesome-icon>
                        </span>
                        <span class="truncate text-sm font-medium text-gray-900 dark:text-stone-300">{{ views }} </span>
                    </li>
                    ...
                    >
                        <font-awesome-icon icon="fa-solid fa-trash" size="lg"> </font-awesome-icon>
                    </button>
                </ul>
            </button>
        </li>
